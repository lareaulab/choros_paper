...
dataset <- "wu_2019"
expts <- setNames(c("CHXTIG_WT", "CHXTIG_3AT"),
                  c("CHXTIG_WT", "CHXTIG_3AT"))
...
bam_names <- setNames(c("WT_CHXTIG", "3AT_CHXTIG"),
                      names(expts))
bam_alignment_fnames <- sapply(expts,
                               function(expt) {
                                 file.path(dataset_dir, "raw_data", bam_names[expt],
                                           grep("transcript.bam",
                                                list.files(file.path(dataset_dir, "raw_data", 
                                                                     bam_names[expt])),
                                                value=T))
                               })
