...
corrected_codon_corr$expt <- factor(corrected_codon_corr$expt, levels=expts)
...
figure_2C <- ggplot(corrected_codon_corr, aes(x=position, y=codon_corr, fill=label)) + 
...
ggsave(filename=file.path(figures_dir, "figure_2C.pdf"),
       plot=figure_2C, device="pdf", width=1.5, height=4, units="in")
